 ...
 *
 * Fix v2.2:
 *  - Fix #1: ps_get_task_state() hash collision protection dengan
 *    time-based guard 500ms — state task aktif tidak akan di-reset
 *    oleh task lain yang hash-nya sama. Semua caller cek NULL.
 *  - Fix #2: ps_task_util_pct() tambah guard ps_detected untuk
 *    early boot safety sebelum ps_silver_cap valid.
 *  - Fix #3: detect_cluster_capacity() ambil ps_gold_max_freq dari
 *    PRIME core (kapasitas tertinggi), bukan sekadar first non-silver.
 *    Penting untuk Snapdragon 3-cluster (silver/gold/prime).
 *  - Fix #4: skip_freq_gate threshold turun 40% → 15% agar hanya
 *    task benar-benar idle-like yang bypass freq gate.
 *  - Fix #5: find_best_silver_cpu() fallback path tambah hard cap
 *    75% util pada best_cpu_fallback untuk cegah silver overloaded.
 */
...
static bool detect_cluster_capacity(void)
{
    ...
    /*
     * v2.2 Fix #3: cache gold_max_freq dari PRIME core (kapasitas tertinggi).
     * Pada Snapdragon 3-cluster (silver/gold/prime), first non-silver
     * bisa saja gold biasa, bukan prime — freq gate jadi terlalu ketat.
     */
    {
        int prime_cpu = -1;
        ...
        if (prime_cpu >= 0) {
            unsigned long gfreq = cpufreq_quick_get_max(prime_cpu);
            if (gfreq) {
                WRITE_ONCE(ps_gold_max_freq, gfreq);
                pr_info("prefer_silver: prime=cpu%d max_freq=%lukHz\n",
                    prime_cpu, gfreq);
            }
        }
    }
...
static int ps_cpu_online_cb(unsigned int cpu)
{
    if (!atomic_read(&ps_detected)) {
        ...
        if (cpu_is_silver(cpu)) {
            ...
            /*
             * v2.2: Refresh ps_gold_max_freq hanya dari CPU dengan
             * kapasitas >= ps_gold_cap (prime-tier).
             */
            unsigned long cur_prime_freq = READ_ONCE(ps_gold_max_freq);
            unsigned long this_cap = capacity_orig_of(cpu);

            if (!cur_prime_freq || this_cap >= ps_gold_cap) {
                unsigned long gfreq = cpufreq_quick_get_max(cpu);
                if (gfreq && gfreq > cur_prime_freq)
                    WRITE_ONCE(ps_gold_max_freq, gfreq);
            }
        }
    }
    ...
    if (s->pid != pid) {
        if (s->pid != 0) {
            ...
            if (last_active &&
                ...
                /* Slot masih aktif, jangan overwrite */
                return NULL;
            }
        }
        /* Slot kosong atau sudah expired */
        s->pid           = pid;
        s->last_big_ts   = 0;
        s->last_burst_ts = 0;
        s->peak_util     = 0;
...
void prefer_silver_update_task(struct task_struct *p, int cpu)
{
    ...
    /*
     * v2.2: ps_get_task_state() bisa return NULL saat hash collision aktif.
     * Skip update — guard default ke READ_ONCE(0) = pass (conservative: boleh silver).
     */
    if (!s) {
...
int find_best_silver_cpu(struct task_struct *p)
{
    ...
    /* Guard 2: task baru saja jalan di big core (80ms guard) */
    if (!ps_check_bigcore_recency(p)) {
        atomic_inc(&ps_miss_count);
        atomic_inc(&ps_miss_bigcore);
        return -1;
    }
    /* Guard 3: task baru saja burst (150ms hysteresis) */
    ...
    /*
     * v2.2 Fix #4: skip_freq_gate threshold turun 40% → 15%.
     * Hanya task benar-benar idle-like yang bypass freq gate.
     */
...
retry:
    ...
    /*
     * v2.2 Fix #5: Fallback path tambah hard cap 75% utilization.
...
int __init prefer_silver_init(void)
{
    ...
    pr_info("prefer_silver v2.2: init OK | enabled=%d heavy=%d%% cpu=%d%% "
