...
/*
 * prefer_silver.c v2.1
 *
 * Fix v2.0:
 *  - prefer_silver_update_task() dipindah setelah semua static helpers
 *  - Tambah Guard 1: uclamp_min > 0 → skip silver
 *  - Tambah Guard 2: big-core recency (80ms)
 *  - Tambah Guard 3: burst hysteresis (150ms)
 *  - Lower heavy_task_thresh: 75 → 55
 *  - Lower cpu_util_thresh: 90 → 85
 *
 * Fix v2.1:
 *  - ps_gold_max_freq diisi saat detect_cluster_capacity() → freq gate aktif
 *  - ps_cpu_online_cb refresh ps_gold_max_freq saat gold CPU online
 *  - ps_check_burst_decay lockless (READ_ONCE) sesuai bigcore_recency
 *  - Fix komentar nilai tunable (big_core_guard=80ms, burst_decay=150ms)
 */
...
int sysctl_heavy_task_thresh = 55;   /* v2.0: turun dari 75 */
int sysctl_cpu_util_thresh   = 85;   /* v2.0: turun dari 90 */
int sysctl_freq_ratio_thresh = 95;

/* v2.0: new tunables */
unsigned long sysctl_big_core_guard_ns = 80000000UL; /* 80ms */
int           sysctl_burst_thresh      = 35;          /* 35% silver_cap */
unsigned long sysctl_burst_decay_ns    = 150000000UL; /* 150ms */
...
/* ------------------------------------------------------------------ *
 * Cluster state — HARUS di atas semua fungsi yang menggunakannya
 * ------------------------------------------------------------------ */
...
/* --- cpu_is_silver: static inline, didefinisikan SEBELUM dipakai --- */
...
static bool detect_cluster_capacity(void)
{
    ...
    for_each_possible_cpu(cpu) {
        ...
        pr_debug("prefer_silver: cpu%d cap=%lu\n", cpu, cap);
        if (cap < min_cap)
            min_cap = cap;
    }
    
    pr_info("prefer_silver: min_cap=%lu threshold=%lu\n",
            min_cap, min_cap * 115 / 100); /* DEBUG */
    ...
    for_each_possible_cpu(cpu) {
        ...
        if (cap <= (min_cap * 115 / 100)) {
            ...
            pr_info("prefer_silver: cpu%d -> silver\n", cpu); /* DEBUG */
        }
    }
    ...
    /* v2.1: cache gold max freq untuk freq gate */
    {
        int first_gold = -1;
        for_each_possible_cpu(cpu) {
        ...
        if (first_gold >= 0) {
            unsigned long gfreq = cpufreq_quick_get_max(first_gold);
            if (gfreq)
    ...
    pr_info("prefer_silver: mask=%*pbl online=%*pbl\n",
            cpumask_pr_args(&ps_silver_mask),
            cpumask_pr_args(&ps_silver_online)); /* DEBUG */

	if (cpumask_empty(&ps_silver_online))
		cpumask_and(&ps_silver_online, &ps_silver_mask,
			    cpu_possible_mask);
	if (cpumask_empty(&ps_silver_online))
		return false;
...
static int ps_cpu_online_cb(unsigned int cpu)
{
    if (!atomic_read(&ps_detected)) {
        ...
        if (cpu_is_silver(cpu)) {
            ...
            /* Refresh gold_max_freq jika belum terisi */
            if (!READ_ONCE(ps_gold_max_freq)) {
...
/* ------------------------------------------------------------------ *
 * Util helpers — HARUS di atas prefer_silver_update_task()
 * ------------------------------------------------------------------ */
...
static inline unsigned long ps_task_util_pct(struct task_struct *p)
{
	...
	unsigned long cap  = ps_silver_cap ?
			     ps_silver_cap : capacity_orig_of(task_cpu(p));
	if (unlikely(!cap))
		return 100;
	return (util * 100) / cap;
}
...
/* ------------------------------------------------------------------ *
 * prefer_silver_update_task — SETELAH semua helper dideklarasikan
 *
 * Error asal muncul karena fungsi ini dulunya ada di ATAS sebelum
 * ps_detected, cpu_is_silver, ps_task_util, dan ps_silver_cap
 * dideklarasikan.
 * ------------------------------------------------------------------ */
void prefer_silver_update_task(struct task_struct *p, int cpu)
{
	...
	is_big = !cpu_is_silver(cpu);   /* OK: cpu_is_silver sudah dideklarasikan */
	util   = ps_task_util(p);       /* OK: ps_task_util sudah dideklarasikan */
	now    = ktime_get_ns();
	...
	if (ps_silver_cap) {            /* OK: ps_silver_cap sudah dideklarasikan */
...
static inline bool ps_check_bigcore_recency(struct task_struct *p)
{
    ...
    last_big = READ_ONCE(ps_task_cache[slot].last_big_ts);  // lockless read
    if (!last_big)
        return true;
...
static void ps_delayed_detect_work(struct work_struct *work)
{
    ...
    if (detect_cluster_capacity()) {
        ...
        /* v2.1: retry tiap 5s jika masih gagal */
...
int __init prefer_silver_init(void)
{
    ...
    /* Coba detect awal — mungkin berhasil jika dipanggil cukup terlambat */
    if (detect_cluster_capacity()) {
        atomic_set(&ps_detected, 1);
    } else {
        /* Schedule ulang 3 detik setelah boot untuk pastikan CPU sudah fully online */
	...
	pr_info("prefer_silver v2.0: init OK | enabled=%d heavy=%d%% cpu=%d%% guard=%lums burst_decay=%lums\n",
		sysctl_prefer_silver,
		sysctl_heavy_task_thresh,
		sysctl_cpu_util_thresh,
